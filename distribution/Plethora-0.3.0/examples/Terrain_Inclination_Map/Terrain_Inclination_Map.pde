...
void setup() {
 ...
 //declare a vector as the location
  ...
  pTer = new Ple_Terrain(this, location,  100,100, 10, 10);
...
void draw() {
  ...
  //calculate the map of inclination
  float [][] angles = pTer.calcSteepnessMap();
  ...
  //draw the data map, specify the map to draw, where vallues go from 0 to PI/2 (90) and draw them on colors from 255 to 0
  pTer.drawDataMap(angles, 0, PI/2,   255,0);
